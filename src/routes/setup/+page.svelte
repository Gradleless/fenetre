				...
				<form onsubmit={(e) => { e.preventDefault(); submit() }}>
					<FieldGroup>
						<Field>
							...
							<Input id="s-name" bind:value={name} placeholder={m['setup.name.placeholder']()} required />
						</Field>
						<Field>
							<FieldLabel for="s-email">{m['login.email_label']()}</FieldLabel>
							<Input id="s-email" type="email" bind:value={email} placeholder={m['login.email_placeholder']()} required />
						</Field>
						<Field>
							<FieldLabel for="s-password">{m['login.password_label']()}</FieldLabel>
							<Input id="s-password" type="password" bind:value={password} placeholder={m['login.password_placeholder']()} minlength={8} required />
						</Field>
