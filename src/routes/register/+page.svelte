				...
				<form onsubmit={(e) => { e.preventDefault(); submit() }}>
					<FieldGroup>
						<Field>
							...
							<Input id="r-name" bind:value={name} placeholder={m['register.name.placeholder']()} required />
						</Field>
						<Field>
							<FieldLabel for="r-email">{m['login.email_label']()}</FieldLabel>
							<Input id="r-email" type="email" bind:value={email} placeholder={m['login.email_placeholder']()} required />
						</Field>
						<Field>
							<FieldLabel for="r-password">{m['login.password_label']()}</FieldLabel>
							<Input id="r-password" type="password" bind:value={password} placeholder={m['login.password_placeholder']()} minlength={8} required />
						</Field>
