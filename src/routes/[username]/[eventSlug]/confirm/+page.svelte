...
<div class="mx-auto max-w-lg px-4 py-16 text-center">
	{#if !booking}
		...
		<Button href="/{username}{embed ? '?embed=1' : ''}" variant="ghost" class="mt-4">{m['booking.back']()}</Button>
	{:else}
		<div class="flex flex-col items-center gap-6">
			...
			<Card.Root class="w-full text-left">
				...
				<Card.Content class="flex flex-col gap-2 text-sm text-muted-foreground">
					...
					<div class="flex items-center gap-2">
						<Clock class="size-4" />
						<span>{formatTime(booking.startTime)} — {formatTime(booking.endTime)} ({booking.eventType.duration} min)</span>
					</div>
				</Card.Content>
			</Card.Root>
			...
			<Button href="/{username}{embed ? '?embed=1' : ''}" variant="outline" class="w-full">{m['confirm.other_types']()}</Button>
		</div>
	{/if}
</div>
