<script lang="ts">
	...
	$effect(() => {
		...
		if (error === 'no_refresh_token') toast.error(m['admin.settings.google.error_no_refresh_token']())
		if (error === 'oauth_invalid_state') toast.error(m['admin.settings.google.error_oauth_state']())
		if (error === 'google_not_configured') toast.error(m['admin.settings.google.error_not_configured']())
	})
	...
	async function toggleBusyMode(value: boolean) {
		...
		try {
			...
			toast.success(value ? m['admin.settings.busy_mode.on']() : m['admin.settings.busy_mode.off']())
		} catch {
...
<div class="flex flex-col gap-6">
	...
	<Card.Root>
		<Card.Content class="flex items-center justify-between gap-4 py-6">
			<div>
				...
				<p class="mt-1 text-sm text-muted-foreground">{m['admin.settings.busy_mode.description']()}</p>
			</div>
	...
	<Card.Root>
		...
		<Card.Content>
			<Button href="/api/google/connect" variant={settings.googleRefreshToken ? 'outline' : 'default'}>
		...
		<Card.Content>
			<FieldGroup>
				...
				<Field>
					...
					<p class="mb-2 text-sm text-muted-foreground">{m['admin.settings.buffer.description']()}</p>
		...
		<Card.Content class="flex flex-col gap-4">
			...
			{:else}
				{#each portfolioLinks as link, i}
					<div class="grid grid-cols-[180px_1fr_1fr_auto] items-end gap-3" transition:slide={{ duration: 150, axis: 'y' }}>
						<Field>
							{#if i === 0}
							...
							<Select.Root
								...
								onValueChange={(v: string) => { portfolioLinks[i].missionType = v }}
							>
								<Select.Trigger>
									{MISSION_OPTIONS.find((o) => o.value === link.missionType)?.label ?? link.missionType}
								</Select.Trigger>
								<Select.Content>
						...
						<Field>
							{#if i === 0}<FieldLabel>{m['admin.settings.portfolio.link_title']()}</FieldLabel>{/if}
							<Input bind:value={portfolioLinks[i].title} placeholder={m['admin.settings.portfolio.link_title']()} />
						</Field>
						<Field>
