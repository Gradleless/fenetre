				...
				<form onsubmit={(e) => { e.preventDefault(); submit() }}>
					<FieldGroup>
						<Field>
							...
							<Input id="rp-password" type="password" placeholder={m['login.password_placeholder']()} bind:value={password} minlength={8} required />
						</Field>
						<Field>
							<FieldLabel for="rp-confirm">{m['reset_password.confirm_label']()}</FieldLabel>
							<Input id="rp-confirm" type="password" placeholder={m['login.password_placeholder']()} bind:value={confirm} minlength={8} required />
						</Field>
