...
export const getAvailableSlots = query(
	...
	async ({ username, eventTypeSlug }) => {
		...
		const [availabilityRows, existingBookings, busySlots] = await Promise.all([
			db.select().from(availability).where(and(eq(availability.userId, userId), eq(availability.isActive, true))),
			db.select().from(bookings).where(and(eq(bookings.userId, userId), gte(bookings.startTime, now), ne(bookings.status, 'cancelled'))),
...
export const getAllAvailability = query(async () => {
	...
	return db.select().from(availability).where(eq(availability.userId, user.id)).orderBy(availability.dayOfWeek)
})
