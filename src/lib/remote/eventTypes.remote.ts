...
export const createEventType = command(
	...
	async (input) => {
		const user = requireAuth()
		const [et] = await db.insert(eventTypes).values({ ...input, userId: user.id }).returning()
...
export const setBusyModeAll = command(z.object({ isBusyMode: z.boolean() }), async ({ isBusyMode }) => {
	const user = requireAuth()
