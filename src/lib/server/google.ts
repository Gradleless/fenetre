...
async function getCalendarClient(userId: string) {
	...
	const [row] = await db
		.select({ googleRefreshToken: userSettings.googleRefreshToken, googleCalendarId: userSettings.googleCalendarId })
		.from(userSettings)
...
export async function getBusySlots(userId: string, timeMin: Date, timeMax: Date): Promise<BusySlot[]> {
...
export async function createCalendarEvent(
	userId: string,
	event: { summary: string; description?: string; startTime: Date; endTime: Date; attendeeEmail: string }
